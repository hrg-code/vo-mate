...
每次完整生成、AI 改写、平台适配或用户点击保存时形成版本；编辑器内逐字修改只保留为当前工作副本，不自动生成版本：
...
版本记录 `parent_version_id`，可以形成 `v1 AI 初稿 -> v2 用户修改 -> v3 抖音版 / v4 小红书版` 的来源链。用户可切换当前版本、标记采用/废弃，并在后续版本对比中查看差异。
